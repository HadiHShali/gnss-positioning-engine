// ====================================================//
// Plot All Visible Satellites using SkyPlot in python //
// ====================================================//
//Computes positions of all visible GPS satellites from a receiver location and then plot them
...
int main()
{
    ...
    ofstream csv("../../data/visible_sats.csv");
    csv << "prn,elevation_deg,azimuth_deg,range_m" << endl;
    for (const auto& v : visible)
    {
        csv << v.prn << "," << v.elevation_deg << "," << v.azimuth_deg << "," << v.range_m << endl;
    }
    csv.close();
    cout << "CSV written to: data/visible_sats.csv" << endl;
